    ...
    fetch(`/api/courses/${id}`)
      ...
      .then((data) => {
        ...
        setForm({
          ...
          enrollCount: c.enrollCount ? String(c.enrollCount) : "",
        });
        ...
        setWhatYouLearn(c.whatYouLearn?.length ? c.whatYouLearn : [""]);
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
          ...
          <div>
            ...
            <textarea rows={4} value={form.description} onChange={(e) => setField("description", e.target.value)} className={inputCls + " resize-none"} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.language} onChange={(e) => setField("language", e.target.value)} className={inputCls} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" min={0} value={form.enrollCount} onChange={(e) => setField("enrollCount", e.target.value)} className={inputCls} />
            </div>
          </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
          ...
          <div>
            ...
            <input value={form.instructor} onChange={(e) => setField("instructor", e.target.value)} className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Instructor Bio</label>
            <textarea rows={3} value={form.instructorBio} onChange={(e) => setField("instructorBio", e.target.value)} placeholder="Short bio..." className={inputCls + " resize-none"} />
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          ...
          {lessons.length === 0 && (
            <p className="text-gray-600 text-sm text-center py-6">No lessons yet.</p>
          )}
            ...
            {lessons.map((lesson, i) => (
              <div key={i} className="bg-gray-800/50 rounded-xl p-4 space-y-3 border border-gray-700/50">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={labelCls}>Title</label>
                    <input value={lesson.title} onChange={(e) => updateLesson(i, "title", e.target.value)} className={inputCls} />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input value={lesson.videoUrl} onChange={(e) => updateLesson(i, "videoUrl", e.target.value)} className={inputCls} />
                </div>
                <div>
                  <label className={labelCls}>Notes / Description</label>
                  <textarea rows={2} value={lesson.description} onChange={(e) => updateLesson(i, "description", e.target.value)} className={inputCls + " resize-none"} />
                </div>
              </div>
            ))}
          </div>
